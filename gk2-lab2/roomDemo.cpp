	...
	m_particles{ {-1.3f, -0.6f, -0.14f} }
{
	...
	XMStoreFloat4x4(&tempMtx,
		...
		XMMatrixRotationZ(XMConvertToRadians(10)) *
		XMMatrixScaling(1, 3.0 / 4.0, 1) * 
...
void mini::gk2::RoomDemo::UpdateParticles(float dt)
{
	...
	vector<ParticleVertex> particles = m_particles.Update(dt, m_camera.getCameraPosition());
	D3D11_MAPPED_SUBRESOURCE mapped;
	auto hr = m_device.context()->Map(m_vbParticles.get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped);
	if (FAILED(hr)) {
		THROW_DX(hr);
	}
	memcpy(mapped.pData, particles.data(), sizeof(ParticleVertex) * particles.size());
	m_device.context()->Unmap(m_vbParticles.get(), 0);
}
...
void RoomDemo::DrawWalls()
{
	...
	XMStoreFloat4x4(&texMtx, XMMatrixTranslation(2, 2, 0) * XMMatrixScaling(0.25, 0.25, 1));
	...
	XMStoreFloat4x4(&texMtx, XMMatrixTranslation(2, 2, 0) * XMMatrixScaling(0.25, 4.0, 1));
	UpdateBuffer(m_cbTex1Mtx, texMtx);
